...
fun byRating(): Comparator<HasRating> = comparingDouble(HasRating::rating).reversed()

fun byPriceLowToHigh(): Comparator<HasPrice> = comparing(HasPrice::price)

fun <T> byValue(): Comparator<T> where T : HasPrice, T : HasRating = comparingDouble { t: T -> t.rating / t.price }.reversed()

fun byRelevance(): Comparator<HasRelevance> = comparingDouble(HasRelevance::relevance).reversed()